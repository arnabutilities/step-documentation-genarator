...
After each run the agent renders a **flow diagram of the top two hierarchy levels** (the root and its direct children) as an SVG saved next to the root document:

```
output/
├── how-to-deploy-a-web-app-to-production.md
├── how-to-deploy-a-web-app-to-production.flow.svg   <-- diagram
└── how-to-deploy-a-web-app-to-production/
    └── ...
```
...
- The current run's diagram is always refreshed to reflect the latest tree.
- **Existing output is backfilled** — any root document in the output directory that doesn't yet have a `.flow.svg` gets one created. This means diagrams are produced for trees built before this feature existed, without re-calling the model (the diagram is built purely from the saved Markdown).

The diagram is generated by `agent/diagram.py`, which reads the root document's title and its ordered `## Detailed Steps` links, then lays them out as a simple top-down org chart (root on top, numbered children below). Generating diagrams never costs API tokens.

---

## Resuming (Incremental Runs)

The agent is **resumable**. If output for a topic already exists in the output directory, re-running the same command **reuses the existing files** (no API calls) and only generates the missing parts of the tree.

This makes it cheap to:

- **Recover from an interruption** (e.g. a crash or rate-limit error mid-run) — just run the same command again.
- **Expand a tree further** — raise `--max-depth` / `--max-nodes` and the agent continues from where it left off instead of regenerating everything.

```bash
# First run (partial — limited budget)
python -m agent.main "How to deploy a web app to production" --max-nodes 5

# Later: continue and go deeper. Existing docs are reused; only new ones are generated.
python -m agent.main "How to deploy a web app to production" --max-nodes 30
```

Each document logs whether it was `(generate)` or `(reuse)`, and the final summary reports both counts. To ignore existing output and rebuild from scratch, pass `--fresh`.

How it works: before calling the model, `expand()` checks whether the target `<slug>.md` already exists. If so, it reads that file (stripping the auto-generated `## Detailed Steps` section so child links can be recomputed) instead of generating it. Recursion then proceeds into children, generating only those that don't yet exist.
...
| `--fresh` | `false` | Ignore existing output and regenerate everything from scratch. |
